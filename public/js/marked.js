...
Renderer.prototype.code = function(code, lang, escaped) {
  ...
  //console.log(code);
  var clText = code.split('::');
  if (!lang) {
    return '<pre class="prettyprint"><code class="lang-'+clText[0]+'">'
      + (escaped ? clText[1] : escape(clText[1], true))
      + '\n</code></pre>';
  }
...
Renderer.prototype.heading = function(text, level, raw) {
  //console.log(text);
  var clText = text.split("::");
  return '<h'
    + level
    + ' class="'
    + this.options.headerPrefix
    + clText[0].toLowerCase().replace(/[^\w]+/, ' ')
    + '">'
    +  clText[1]
    + '</h'
    + level
...
Renderer.prototype.paragraph = function(text) {
  //console.log(text);
  var clText = text.split("::");
  return '<p class="'+clText[0]+'" >' + clText[1] + '</p>\n';
};
